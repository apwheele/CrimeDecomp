...
```powershell
powershell -ExecutionPolicy Bypass -File src/run_scheduled_release.ps1
```
...
`src/data/model/scheduled-release.log`. A preflight without rendering, committing,
or deploying is available with `-PreflightOnly`.

Install the current-user Windows Task Scheduler entry (every Monday at
9:00 AM by default) with:

```powershell
powershell -ExecutionPolicy Bypass -File src/install_weekly_release_task.ps1
```

The installer accepts `-DayOfWeek`, `-At`, and `-TaskName`. The scheduled task
